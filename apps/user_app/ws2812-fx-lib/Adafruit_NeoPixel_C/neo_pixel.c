...
// #define SYS_MAX_LED_NUMBER (u32)300
#define SYS_MAX_LED_NUMBER (u32)1000
...
void ws281x_show(unsigned char *pixels_pattern, unsigned short pattern_size)
{
  ...
  for(i=0; i< pattern_size/3; i++)
  {
    ...
    memcpy(buf, pixels_pattern, fc_effect.led_num * 3); // fc_effect.led_num * 3，灯的数量 * rgb

    // 如果灯光关闭，清空数据，不显示颜色
    if (0 == save_info.flag_is_light_on)
    {
        memset(buf, 0, fc_effect.led_num * 3); // fc_effect.led_num * 3，灯的数量 * rgb
    }
